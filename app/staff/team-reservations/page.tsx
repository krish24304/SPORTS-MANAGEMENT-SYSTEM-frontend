...
const [resourceUnits, setResourceUnits] =
  useState<any[]>([]);
...
setSelectedResources] =
  ...
  fetch(
    `http://localhost:5000/resource-units/sport/${sportId}`
  )
    .then((res) => res.json())
    .then((data) => {

      setResourceUnits(data);
  ...
  alert("Fill all fields and select at least one resource");
  return;
}
      const user =
        JSON.parse(
